...
| [control-arbitration.md](control-arbitration.md) | Loop outputs, hard limits, combiner, anti-windup | `mf_climate_arbiter`, `mf_loop_rh`, `mf_loop_co2`, `mf_loop_temp`, `mf_control_limits`, `mf_pid`, `mf_control_profile`, `mf_climate_trace` | Golden vectors in [s5-acceptance.md](../dev/acceptance/s5-acceptance.md); mock scenarios via `mf_mock_climate`; bench tuning B1–B7 in roadmap |
| Roadmap **S5** (climate + arbitration) | RH/CO2/TEMP loops, hard limits, cooperative combiner, decision trace | modules above + `mf_climate` | Phase A: all rows in [s5-acceptance.md](../dev/acceptance/s5-acceptance.md) checklist; Phase B: contour stable on bench 30+ min |
...
| [../api/mqtt-contract.md](../api/mqtt-contract.md) | MQTT envelope, topics, commands, idempotency | `mf_mqtt`, `mf_mqtt_config`, `mf_cmd_dispatch`, `mf_msg_dedup` | [s7-acceptance.md](../dev/acceptance/s7-acceptance.md) T1–T9; `mushfarm_tests` suite 5 (dedup + dispatch) |
| [../api/ap-contract.md](../api/ap-contract.md) | HTTP parity with MQTT (subset in S6) | `mf_http_api`, `mf_net_config`, `mf_wifi` | [s6-acceptance.md](../dev/acceptance/s6-acceptance.md) curl checklist M1–M8 |
